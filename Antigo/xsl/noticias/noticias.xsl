  ...
  <xsl:include href="/_layouts/xsl/internal.xsl"/> 
	...
	<xsl:template match="/" ddwrt:ghost="" xmlns:x="http://www.w3.org/2001/XMLSchema" xmlns:d="http://schemas.microsoft.com/sharepoint/dsp" xmlns:asp="http://schemas.microsoft.com/ASPNET/20" xmlns:__designer="http://schemas.microsoft.com/WebParts/v2/DataView/designer" xmlns:SharePoint="Microsoft.SharePoint.WebControls" xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:ddwrt="http://schemas.microsoft.com/WebParts/v2/DataView/runtime" >	
		...
		<div class="noticias-container">
			<div class="rihappy-webparts-titulo">Notícias</div>		
			<xsl:for-each select="/dsQueryResponse/Rows/Row">
				<div class="noticia-item">
					<a href="javascript:OpenPopUpPageWithTitle('/Paginas/NoticiaDetalhes.aspx?IDNoticia={@ID}', null, null, null,'{@Title}')">								
					...
					<xsl:choose>
						<xsl:when test="@imagem != ''">
							...
							<div class="noticia-titulo"><xsl:value-of select="substring(@Title,0,254)"></xsl:value-of></div>
							<div class="noticia-descricao"><xsl:value-of select="substring(@Resumo, 0, 33)"></xsl:value-of></div>
						</xsl:when>
						<xsl:otherwise>							
							<div class="noticia-data">
								<xsl:call-template name="inpartec_formatdate">
							...
							<div class="noticia-titulo"><xsl:value-of select="substring(@Title,0,254)"></xsl:value-of></div>
							<div class="noticia-descricao"><xsl:value-of select="substring(@Resumo, 0, 254)"></xsl:value-of></div>						
						</xsl:otherwise>					
					</xsl:choose>
					</a>																		
				</div>							
			</xsl:for-each>
			<div class="noticia-ver-todas"><a href="/Paginas/Todas-Noticias.aspx">ver todas</a></div>		
		</div>
	</xsl:template> 
