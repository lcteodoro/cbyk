...
topNav = function () {
	...
	var config = {
		siteUrl: 'https://rihappy365.sharepoint.com/sites/developer', // blank siteUrl node, this will try to auto-determine
	...
	var loadNavigation = function (targetSelector) {
		...
		$.when(getNodesPromise).done(function(navigationNodes) {
			...
			$(".nav.navbar-nav.navbar-right").append('<li class="[ hidden-xs ]"><a href="#toggle-search" class="[ animate ]"><span class="[ glyphicon glyphicon-search ]"></span></a></li><li class="[ hidden-xs ]"><a href="#toggle-toolbar" class="[ animate ]"><span class="[ glyphicon glyphicon-th ]"></span></a></li>');
			
			//$(".nav.navbar-nav.navbar-right").append('<li class="[ visible-xs ]"><form action="/sites/developer/_layouts/15/osssearchresults.aspx" method="GET" role="search"><div class="[ input-group ]"><input type="text" class="[ form-control ]" name="u" placeholder="Buscar"><span class="[ input-group-btn ]"><button class="[ btn btn-primary ]" type="submit"><span class="[ glyphicon glyphicon-search ]"></span></button><button class="[ btn btn-danger ]" type="reset"><span class="[ glyphicon glyphicon-remove ]"></span></button></span></div></form></li>');
		
			$('a[href="#toggle-toolbar"], .navbar-bootsnipp .bootsnipp-toolbar .input-group-btn > .btn[type="reset"]').on('click', function(event) {
				event.preventDefault();
				$('.navbar-bootsnipp .bootsnipp-toolbar .input-group > input').val('');
				$('.navbar-bootsnipp .bootsnipp-toolbar').toggleClass('open');
				$('a[href="#toggle-toolbar"]').closest('li').toggleClass('active');

				if ($('.navbar-bootsnipp .bootsnipp-toolbar').hasClass('open')) {
					/* I think .focus dosen't like css animations, set timeout to make sure input gets focus */
					setTimeout(function() {
						$('.navbar-bootsnipp .bootsnipp-toolbar .form-control').focus();
					}, 100);
				}
			});
	...
	var renderNavigationNodes = function (navigationNodes, targetSelector, isSub) {
		...
		var nodeGroupProps = isSub ? { class: "dropdown-menu", role: "menu" } : { class: "nav navbar-nav navbar-right" };
		...
		$.each(navigationNodes, function (index, item) {
			...
			var idJslist = item.Key.split("=");
			
			var idJs = 0;
			
			if(idJslist.length > 0){
				idJs = idJslist[1];
			}else{
				idJs = idJslist;
			}
			...
			$.extend(navItemProps, {
				id: idJs // give the navigation item an ID using the loop's index
			});
			...
			if(hasChildren) {
				...
				renderNavigationNodes(item.Nodes.results, "#" + idJs, true);
			}
		});
...
$(document).ready(function() {
	...
	topNav.LoadNavigation("#bs-example-navbar-collapse-1");  // the top level div where to put the navgiation nodes
});
